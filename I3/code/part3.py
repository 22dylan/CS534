...
path_to_val = os.path.join(os.getcwd(), '..', 'data', 'pa3_val.csv')
...
wrong_train_all = [] #how many wrong guesses on training data
wrong_val_all = []  #how many wrong guesses on validation data
...
for L in learners:
    ...
    howmanywrong_train = 0
    for index,ex in data.iterrows():
        guess = toEatOrNotToEat(H,Alpha,ex)
        if guess != ex[y]:
            howmanywrong_train += 1
    wrong_train_all.append(howmanywrong_train)
    print('{} wrong on training data'.format(howmanywrong_train))
    ...
    for index,ex in val_data.iterrows():
        ...
        if guess != ex[val_y]:
            howmanywrong_val += 1
    wrong_val_all.append(howmanywrong_val)
    print('{} wrong on validation data'.format(howmanywrong_val))
...
H = H_all[best_adaboost]
Alpha = Alpha_all[best_adaboost]

#get test data
path_to_test = os.path.join(os.getcwd(), '..', 'data', 'pa3_test.csv')
...
#save test data
path_to_outfile = os.path.join(os.getcwd(), '..', 'output', 'pa3_test_predictions_p3.csv')
