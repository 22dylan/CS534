...
# --- reading in weights ---
path_to_weights = os.path.join('..', 'output', 'w_p2.csv')
weights_all = np.genfromtxt(path_to_weights, delimiter =',')

# --- testing model --- 
iter_selection = 15								# iteration with highest accuracy
w = weights_all[iter_selection-1, :]			# selecting weights

y_hat = np.sign(w.dot(np.transpose(data)))								# testing model
file_out = os.path.join(os.getcwd(), '..', 'output', 'aplabel.csv') # path to output
